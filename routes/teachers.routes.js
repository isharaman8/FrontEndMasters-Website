...
body("description").isLength({min : 10}).withMessage("Please enter a valid description"),
body("current_status").isIn(["Past", "Active"]).withMessage("Please enter current status from Pasr or Active"),
async(req, res)=>{
    try {
...
router.patch("/:id", async(req, res)=>{
    try {
        const teachers = await Teacher.findByIdAndUpdate(req.params.id, req.body).lean().exec();
        return res.status(500).send(teachers);
